...
async function getPropertyData(filter){
  ...
  try {
    ...
    return allProperties.filter((x) => {
      const val = x && x.COMMERCIAL;
      let isCommercial = String(val).toLowerCase() === "true";
      if (!isCommercial) return false;                     // always require commercial
...
function startSlideshow(images) {
  ...
  }, 2300); // 3 seconds — adjust as desired
}
...
async function runCommercial(){
  ...
  setupFilters();
  renderProperties(currentFilter).catch((e) => {alert("Error rendering properties: page will reload."); location.href = "/index.html";});  
}
